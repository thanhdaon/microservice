...
async function run() {
  const conn = await amqp.connect('amqp://admin:Mektoube2020@localhost:5672');
  ...
  await channel.assertQueue('moderation-manager-image-queue', {durable: true});
  await channel.assertQueue('moderation-copy-done', {durable: true});
  await channel.assertQueue(
      'moderation-image-waiting-for-crop-queue', {durable: true});
}
